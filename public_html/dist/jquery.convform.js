...
ConvState.prototype.printQuestion = function () {
    ...
    setTimeout(function () {
        ...
        if (this.current.input.type == "radio") {

            //this.printAnswers1(this.current.input.answers);
        }
        this.scrollDown();
    ...
    var str1 = question;
    var str2 = "Toggle Switch";
    var str3 = "music";
    if (str1.indexOf(str2) != -1)
        inputCondition('hide', 'btn');
    else if (str1.indexOf(str3) != -1)
        inputCondition('multi', 'btn');
    else
        inputCondition('show', 'btn');
...
ConvState.prototype.printAnswers = function (answers, multiple) {
    ...
    if (multiple) {
        ...
        for (var i in answers) {
            if (answers.hasOwnProperty(i)) {
                var opt = $('<div class="opt">' + answers[i].text + '</div>')
                        .data("answer", answers[i])
                        .click(function (event) {
                            ...
                            this.wrapper.find('div.options div.opt').remove();
                        }.bind(this));
                this.wrapper.find('div.options').append(opt);
                $(window).trigger('dragreset');
            }
        }
        var type = 'hide';
...
function inputCondition(type, btn)
{
    if (type == 'show')
    {
        $('#userInput').show();
...
function customValueSubmit(object) {
    var answerText = $(".acidjs-rating-stars").find('input[type=radio]:checked').val();
    var answerObject = answerText;
...
(function ($) {
    $.fn.convform = function (options) {
        ...
        var inputs = $(this).find('input, select, textarea').map(function () {
            ...
            if (input['type'] === "radio") {
                var answer = {};
                answer['text'] = $(".acidjs-rating-stars").find('input[type=radio]:checked').text();
                answer['value'] = $(".acidjs-rating-stars").find('input[type=radio]:checked').val();
            }
        ...
        if (inputs.length) {
            ...
            switch (parameters.typeInputUi) {
                case 'input':
                    inputForm = $('<form id="' + parameters.formIdName + '" class="convFormDynamic"><div class="options dragscmessage to readyroll mycls"></div><input id="' + parameters.inputIdName + '" type="text" placeholder="' + parameters.placeHolder + '" class="userInputDynamic"></><button type="submit" class="submit>⯈</button><span class="clear"></span></form>');
                    break;
                case 'textarea':
            ...
            $(inputForm).find(parameters.inputIdHashTagName).keypress(function (e) {
                if (e.which == 13) {
                    ...
                    if (state.current.input.type == "radio") {
                        // var input = $(this).val();

                        var results = state.current.input.answer.filter(function (el) {
                            return el.text.toLowerCase().indexOf(input.toLowerCase()) != -1;
                        });
                        if (results.length) {
                            //state.current.input.checked = results[0];
                ...
                if (state.current.input.type == "select") {
                ...
                } else if (state.current.input.type == "radio") {
                    // var input = $(this).val();
                    ...
                    if (results.length) {
                        ...
                        state.printAnswers(results, state.current.input.checked);
                    } else {
                        state.wrapper.find(parameters.inputIdHashTagName).addClass('error');
                    }
                }
            ...
            $(inputForm).find('button.submit').click(function (e) {
                ...
                if (state.current.input.type == "select" && !state.current.input.multiple) {
                    if (state.current.input.required) {
                        ...
                        if (input == parameters.placeHolder)
                            input = '';
                        var results = state.current.input.answers.filter(function (el) {
                ...
                {
                    var value = $(".acidjs-rating-stars").find('input[type=radio]:checked').val();
                    var results = value;
                    if (results.length) {
                        $(this).parent('form').submit();
                    }
                } else if (state.current.input.type == "text" && $(state.current.input).attr('name') == 'date') {
                    var dateS = $('#datePick').val();
                    if (dateS.length) {
                        $(this).parent('form').submit();
                    }
                } else if (state.current.input.type == "checkbox" && $(state.current.input).attr('name') == 'togg') {
                    var toggle1 = $(".chk1").is(":checked");
                    var toggle2 = $(".chk2").is(":checked");
                    ...
                    var toggle = $(".switch").find("input[type=checkbox]").val(toggle1 + toggle2);
                    if (toggle.length) {
                        $(this).parent('form').submit();
                    }
                } else {
            ...
            $(inputForm).submit(function (e) {
                ...
                {
                    var value = $(".acidjs-rating-stars").find('input[type=radio]:checked').val();
                    state.answerWith(value, value);

                } else if (state.current.input.type == "text" && $(state.current.input).attr('name') == 'date') {
                    var dateS = $('#datePick').val();
                    state.answerWith(dateS, dateS);
                } else if (state.current.input.type == "checkbox" && $(state.current.input).attr('name') == 'togg') {
                    var toggle = $(".switch").find('input[type=checkbox]').val();

                    state.answerWith(toggle, toggle);
                } else {
